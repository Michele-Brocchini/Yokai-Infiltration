...
class Player {
  ...
  constructor(public username: string) {
    ...
    this.element = $(
      '<div class="gamePlayer">' +
        '<div class="gamePlayerIcon"></div>' +
        '<div class="gamePlayerName"></div>' +
      '</div>'
    );

    this.element.find(".gamePlayerName").text(username);

    $("#playerBoard").append(this.element);
  ...
  updateAppearance() {
    ...
    this.element.toggleClass("voted", this.votedFor);
  }
...
user.socket.on("hang", function(usernames: Array<string>) {
  ...
  $("#gameMessage").text("Judgment has been passed.");
});
